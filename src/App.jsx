...
export default function App() {
    ...
    <div className="space-y-6">
      <div className="bg-slate-900 text-white p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden">
        <div className="relative z-10">
          ...
          <h2 className="text-5xl font-black tracking-tighter">R$ {(t.rR - t.dR).toLocaleString('pt-BR', {minimumFractionDigits: 2})}</h2>
      ...
      <div className="grid grid-cols-1 gap-4">
        {['essenciais', 'qualidade', 'futuro'].map((b) => {
          const mOrc = (t.rR || t.rP || 1) * (b==='essenciais'?0.6:b==='qualidade'?0.3:0.1);
          const mGasto = despesasReais.filter(d => d.data.startsWith(mesAtual) && categoriasPorBolso[b].includes(d.categoria)).reduce((s, d) => s + d.valor, 0);
          const pct = Math.min((mGasto / mOrc) * 100, 100);
          const cMap = {essenciais: 'bg-blue-500 text-blue-600', qualidade: 'bg-emerald-500 text-emerald-600', futuro: 'bg-violet-500 text-violet-600'};
          return (
            <div key={b} className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100">
              <div className="flex justify-between items-center mb-2">
                <p className={`text-[9px] font-black uppercase tracking-wider ${cMap[b].split(' ')[1]}`}>{b}</p>
                <p className="text-[10px] font-black text-slate-400 font-mono">META: R$ {mOrc.toFixed(0)}</p>
              </div>
              <div className="w-full bg-slate-50 h-3 rounded-full overflow-hidden border border-slate-100">
                <div className={`h-full ${cMap[b].split(' ')[0]} transition-all duration-1000`} style={{ width: `${pct}%` }}></div>
              </div>
              <p className="text-[10px] mt-3 text-slate-500 font-bold flex justify-between uppercase"><span>Gasto:</span> <span className="text-slate-900 font-black">R$ {mGasto.toLocaleString()}</span></p>
            </div>
          );
        })}
      </div>
    ...
    return (
      <div className="space-y-4 pb-28">
        <div className="bg-white p-7 rounded-[2rem] shadow-xl border border-slate-100">
          <h3 className={`font-black mb-4 flex items-center gap-2 uppercase text-[10px] tracking-widest ${isD?'text-orange-600':'text-cyan-600'}`}><Target size={18}/> Meta Mensal</h3>
          <div className="space-y-2">
            {isD && <select className="w-full p-4 border-2 border-slate-100 rounded-2xl bg-slate-50 font-bold" value={f.categoria} onChange={e=>sf({...f, categoria:e.target.value})}>{todasCategorias.map(c=><option key={c} value={c}>{c}</option>)}</select>}
        ...
        <div className="bg-white rounded-[2rem] border border-slate-100 overflow-hidden divide-y">
          {lista.map(i => (
            <div key={i.id} className="p-4 flex justify-between items-center bg-white">
              <div><p className="font-bold text-sm text-slate-800">{i.descricao}</p>{isD && <p className="text-[9px] uppercase font-black text-gray-400">{i.categoria}</p>}</div>
          ...
          <div className={`p-5 font-black flex justify-between items-center uppercase text-[10px] tracking-widest ${isD?'bg-orange-50 text-orange-700':'bg-cyan-50 text-cyan-700'}`}>
  ...
  const LançarReal = ({ tipo }) => {
    ...
    return (
      <div className="space-y-4 pb-28">
        <div className="bg-white p-7 rounded-[2rem] shadow-xl border border-slate-100">
          <h3 className={`font-black mb-4 flex items-center gap-2 uppercase text-[10px] ${isD?'text-rose-600':'text-emerald-600'}`}>{isD?<TrendingDown/>:<PlusCircle/>} Registro Real</h3>
          <div className="space-y-2">
            {isD && <select className="w-full p-4 border-2 border-slate-100 rounded-2xl bg-slate-50 font-bold" value={f.categoria} onChange={e=>sf({...f, categoria:e.target.value})}>{todasCategorias.map(c=><option key={c} value={c}>{c}</option>)}</select>}
            <input className="w-full p-4 border-2 border-slate-100 rounded-2xl outline-none" placeholder="Descrição" value={f.descricao} onChange={e=>sf({...f, descricao:e.target.value})} />
            <input className="w-full p-4 border-2 border-slate-100 rounded-2xl outline-none" type="number" placeholder="R$" value={f.valor} onChange={e=>sf({...f, valor:e.target.value})} />
            <input className="w-full p-4 border-2 border-slate-100 rounded-2xl font-bold" type="date" value={f.data} onChange={e=>sf({...f, data:e.target.value})} />
            <button onClick={handleAdd} className={`w-full text-white p-4 rounded-2xl font-black text-xs uppercase shadow-lg ${isD?'bg-rose-600':'bg-emerald-600'}`}>Registrar</button>
          </div>
        </div>
        <div className="bg-white rounded-[2rem] border border-slate-100 overflow-hidden divide-y">
          {lista.slice().reverse().map(i => (
            <div key={i.id} className="p-4 flex justify-between items-center hover:bg-slate-50">
              <div className="flex items-center gap-3"><div className={`p-2 rounded-xl ${isD?'bg-rose-50 text-rose-500':'bg-emerald-50 text-emerald-500'}`}>{isD?<TrendingDown size={14}/>:<TrendingUp size={14}/>}</div><div><p className="font-black text-slate-800 text-sm">{i.descricao}</p><p className="text-[9px] font-black text-gray-400">{i.data}</p></div></div>
          ...
          <div className={`p-6 font-black flex justify-between items-center uppercase text-[10px] tracking-widest ${isD?'bg-rose-50 text-rose-600':'bg-emerald-50 text-emerald-600'}`}><span>Total Acumulado:</span><span>R$ {lista.reduce((s,i)=>s+i.valor,0).toLocaleString('pt-BR',{minimumFractionDigits: 2})}</span></div>
        </div>
      </div>
    );
  };
  ...
  const Relatorios = () => {
    const pizzaData = [{ name: 'Essenciais', v: (despesasReais.filter(d => d.data.startsWith(mesAtual) && categoriasPorBolso.essenciais.includes(d.categoria)).reduce((s, d) => s + d.valor, 0)) || 0.0001 }, { name: 'Qualidade', v: (despesasReais.filter(d => d.data.startsWith(mesAtual) && categoriasPorBolso.qualidade.includes(d.categoria)).reduce((s, d) => s + d.valor, 0)) || 0.0001 }, { name: 'Investimento', v: (despesasReais.filter(d => d.data.startsWith(mesAtual) && categoriasPorBolso.futuro.includes(d.categoria)).reduce((s, d) => s + d.valor, 0)) || 0.0001 }];
    return (
      <div className="space-y-6 pb-28">
        <div className="bg-white p-8 rounded-[2.5rem] border shadow-xl">
          <h3 className="font-black text-slate-800 mb-8 text-center uppercase tracking-widest text-[10px] opacity-40 italic">Distribuição de Gastos Realizados</h3>
    ...
    return (
      <div className="space-y-6 pb-32">
        <div className="bg-white rounded-[2rem] shadow-xl border overflow-hidden">
          <table className="w-full text-[10px] text-center uppercase font-black">
            ...
            <tbody className="divide-y font-bold">{dados.map(i=>(
              <tr key={i.n}><td className="p-4 text-left">{i.n}</td><td className="text-emerald-600">R$ {i.r.toFixed(0)}</td><td className="text-rose-600">R$ {i.d.toFixed(0)}</td><td className={i.s>=0?'text-blue-600':'text-orange-600'}>R$ {i.s.toFixed(0)}</td></tr>
            ))}</tbody>
          </table>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div className="bg-blue-900 p-5 rounded-3xl text-white flex justify-between shadow-xl"><div><p className="text-[9px] font-black text-blue-300">SALDO TOTAL</p><p className="text-xl font-black">R$ {(totR-totD).toLocaleString()}</p></div><Calculator className="text-blue-500"/></div>
        </div>
      </div>
    );
  };
  ...
  if (!user) {
    return (
      <div className="h-screen flex items-center justify-center bg-slate-900 p-6">
        <div className="bg-white p-12 rounded-[3.5rem] shadow-2xl w-full max-w-sm space-y-8">
          <div className="text-center"><div className="bg-slate-900 w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-xl"><PiggyBank size={32} className="text-white"/></div><h2 className="text-3xl font-black text-slate-900 tracking-tighter">Finance A&H</h2><p className="text-slate-400 font-bold text-[9px] uppercase mt-2 italic tracking-tighter">Budget Manager 2026</p></div>
          <div className="space-y-3">
            ...
            <div className="relative"><input className="w-full border-2 border-slate-100 p-4 rounded-2xl outline-none focus:border-slate-900 bg-slate-50 font-black text-sm transition-all" type={verSenha?'text':'password'} placeholder="SENHA" value={senha} onChange={e=>setSenha(e.target.value)} /><button onClick={()=>setVerSenha(!verSenha)} className="absolute right-4 top-5 text-slate-400">{verSenha?<EyeOff size={20}/>:<Eye size={20}/>}</button></div>
          </div>
          <button onClick={() => signInWithEmailAndPassword(auth, email, senha).catch(()=>alert('Erro!'))} className="w-full bg-slate-900 text-white p-5 rounded-2xl font-black shadow-xl uppercase tracking-widest text-[10px]">Entrar</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      <header className="bg-white/90 backdrop-blur-xl p-5 border-b flex justify-between items-center sticky top-0 z-50">
        <div className="flex flex-col leading-none font-black uppercase italic"><h1 className="text-lg text-slate-900">A&H Finance</h1><p className="text-[8px] text-slate-400 mt-1 italic tracking-widest">Online • <button onClick={()=>signOut(auth)} className="text-rose-500">Sair</button></p></div>
